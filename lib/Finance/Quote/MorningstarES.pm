...
#  MorningstarES.pm
#
#  Obtains quotes for ES Unit Trusts from http://morningstar.es/ - please
#  refer to the end of this file for further information.
#
#  author: Ferran Pallarès (pallares.ferran@gmail.com)
#
#  version: 0.1 Initial version - 27 February 2020
#
...
# URLs
use vars qw($VERSION $MSTARES_NEXT_URL $MSTARES_LOOK_UP $MSTARES_MAIN_URL);
...
$MSTARES_MAIN_URL   =	"http://www.morningstar.es";
$MSTARES_LOOK_UP    =	"http://www.morningstar.es/es/funds/SecuritySearchResults.aspx?search=";
$MSTARES_NEXT_URL   =	"http://www.morningstar.es/es/funds/snapshot/snapshot.aspx?id=";

# FIXME -

sub methods { return (morningstares => \&morningstares_fund); }
...
# perform the look-up - if not found, return with error
        ...
        my $webdoc  = get($MSTARES_LOOK_UP.$code);
        if (!$webdoc)
        {
	    ...
	    $fundquote {$code, "source"} = $MSTARES_MAIN_URL;
...
# Now need to look-up next page using $next_url
        ...
        $webdoc  = get($MSTARES_MAIN_URL.$nexturl);
        if (!$webdoc)
        {
	        # serious error, report it and give up
		    $fundquote {$code,"success"} = 0;
...
# Find date, currency and price all in one table row
		...
		if ($webdoc =~
		m[<td class="line heading">VL<span class="heading"><br />([0-9]{2}/[0-9]{2}/[0-9]{4})</span>.*([A-Z]{3}).([0-9\,\.]+).*>([0-9\,\.\-]+)%?] )
        {
...
# Set a dummy time as gnucash insists on having a valid format
		...
		$fundquote {$code, "method"} = "morningstares";   # set method
...
Finance::Quote::morningstares - Obtain ES Unit Trust quotes from morningstar.es.

=head1 SYNOPSIS

    $q = Finance::Quote->new;

    %info = Finance::Quote->fetch("morningstares","<isin> ...");  # Only query morningstar.es using ISINs

=head1 DESCRIPTION

This module fetches information from the MorningStar Funds service,
http://morningstar.com/es/.

Funds are identified by their ISIN code.

This module is loaded by default on a Finance::Quote object. It's
also possible to load it explicity by placing "morningstares" in the argument
list to Finance::Quote->new().

Information obtained by this module may be covered by Morningstar
terms and conditions See http://morningstar.es/ for details.

=head2 Stocks And Indices

This module provides the "morningstares" fetch method for fetching ES Unit
Trusts and OEICs prices and other information from morningstar.es.

=head1 LABELS RETURNED

The following labels may be returned by Finance::Quote::morningstares :
...
Morning Star websites, http://morningstar.es


=head1 AUTHOR

Ferran Pallarès, E<lt>pallares.ferran@gmail.com<gt>
Based heavily on the work of Martin Sadler E<lt>martinsadler@users.sourceforge.netE<gt>, many thanks!

=head1 COPYRIGHT AND LICENSE

Copyright (C) 2020 by Ferran Pallarès
